...
export default function DomainPage({ domainId, title, subtitle, subDomains }: Props) {
  const { skills, projects, experience } = useResumeStore();

  const domainSkills = filterByDomain(skills, domainId);
  const domainProjects = filterByDomain(projects, domainId);
  const domainExperience = filterByDomain(experience, domainId);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      {domainExperience.length > 0 && (
        <section className="mb-10">
          <h2 className="text-xl font-semibold text-cyan-400 mb-4">Experience</h2>
          {domainExperience.map((exp) => (
            <div key={exp.id} className="mb-6 bg-slate-800 border border-slate-700 rounded-xl p-5">
              ...
              <ul className="list-disc list-inside space-y-1">
                {filterBulletsByDomain(exp.bullets, domainId).map((b) => (
      ...
      {domainSkills.length === 0 && domainProjects.length === 0 && domainExperience.length === 0 && (
